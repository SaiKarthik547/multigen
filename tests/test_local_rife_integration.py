...
def test_local_rife_interpolation_factor_2(mock_frames, tmp_path):
    ...
    # Now returns paths
    assert isinstance(interpolated[0], str)
    
    # Check that the intermediate frame has correct dimensions
    with Image.open(interpolated[1]) as mid:
        assert mid.size == (128, 128)

def test_local_rife_interpolation_factor_4(mock_frames, tmp_path):
